...
def is_rtdetr_model(yolo_name):
    return "rtdetr" in str(yolo_name).lower()
...
def default_imgsz(yolo_name):
    if is_ultralytics_model(yolo_name):
        ...
        return [800, 1440]
    elif is_rtdetr_model(yolo_name):
        return [640, 640]
    else:
        return [640, 640]
...
def get_yolo_inferer(yolo_model):
    ...
    if is_yolox_model(yolo_model):
    ...
    elif is_rtdetr_model(yolo_model):
        from .rtdetr import RTDETRStrategy
        ...
        LOGGER.error("Your model name has to contain either yolox, yolo_nas, yolov8 or rtdetr")
        exit()
